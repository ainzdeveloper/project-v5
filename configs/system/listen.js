...
console.log(`${crayon(``)}${sky(`[ 𝙳𝙰𝚃𝙰 ]`)} bot name : ${chalk.blueBright((!global.config.BOTNAME) ? "𝙰𝙸𝙽𝚉" : global.config.BOTNAME)} \n${sky(`[ 𝙳𝙰𝚃𝙰 ]`)} bot id : ${chalk.blueBright(api.getCurrentUserID())} \n${sky(`[ 𝙳𝙰𝚃𝙰 ]`)} bot prefix : ${chalk.blueBright(global.config.PREFIX)}\n${sky(`[ 𝙳𝙰𝚃𝙰 ]`)} deployed ${chalk.blueBright(operator)} bot operators and ${chalk.blueBright(admin)} admins`);
if (global.config.approval) {
  console.log(`${sky(`[ 𝙳𝙰𝚃𝙰 ]`)} deployed ${chalk.blueBright(approved)} approved groups`)
} 
